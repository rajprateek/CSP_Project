...
def consistent(assignment, csp, var, value):
	"""Question 1"""
	"""YOUR CODE HERE"""
	a = csp.binaryConstraints
	for i in a:
		if(i.affects(var) and i.otherVariable(var) != None):
			if(value == assignment.assignedValues[i.otherVariable(var)]):
				return False
	return True
"""
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in vars:
		if(assignment.assignedValues[var] == None):
			if( minDomain > len(domains[var])):
				nextVar = []
				minDomain = len(domains[var])
				nextVar.append(var)
			elif(minDomain == len(domains[var])):
				nextVar.append(var)
	if(len(nextVar) == 0):
		return None
	if(len(nextVar) == 1):
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	for value in values:
		count=0
		for otherVar in dependents:
			if ( value in assignment.varDomains[otherVar]):
				count=count+1
		constraintNo.append((value,count))
	constraintNo.sort(key=lambda tup: tup[1])
	for tupl in constraintNo:
...
def forwardChecking(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		count=0
		if (constraint.affects(var)):
			for value2 in assignment.varDomains[constraint.otherVariable(var)]:
				if(assignment.assignedValues[constraint.otherVariable(var)]==None and constraint.isSatisfied(value,value2)==False):
					count=count+1
					rvars.append((constraint.otherVariable(var),value2))
				if (count==len(assignment.varDomains[constraint.otherVariable(var)])):
					return None													
	for vars1 in rvars:
		oldDomain = assignment.varDomains[vars1[0]]
		oldDomain.remove(vars1[1])
		assignment.varDomains[vars1[0]] = oldDomain
		inferences.add((vars1[0],vars1[1]))			
	return inferences
"""
...
def revise(assignment, csp, var1, var2, constraint):
	inferences = set([])
	if (constraint.affects(var1)==False or constraint.affects(var2)==False):
		return inferences
	count=0
	for val2 in assignment.varDomains[var2]:
		flag=False
		for val1 in assignment.varDomains[var1]:
			if constraint.isSatisfied(val1,val2)==True:
				flag=True
				break
		if(flag==False):
	...
	for tupl in inferences:
		oldDomain = assignment.varDomains[tupl[0]]
		oldDomain.remove(tupl[1])
		assignment.varDomains[tupl[0]] = oldDomain
	return inferences						
"""
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if (constraint.affects(var) and assignment.assignedValues[constraint.otherVariable(var)]==None):
			q.put((var,constraint.otherVariable(var),constraint))
	while(not q.empty()):
		tupl = q.get()
		inference2=revise(assignment, csp,tupl[0],tupl[1],tupl[2])
		if(inference2==None):
			...
			for constraint in csp.binaryConstraints:
				if (constraint.affects(tupl[1]) and assignment.assignedValues[constraint.otherVariable(tupl[1])]==None):
					t = (tupl[1],constraint.otherVariable(tupl[1]),constraint)
					q.put(t)
